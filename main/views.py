    ...
    if request.method == 'POST':
        if 'update_profile' in request.POST:
            ...
            if ext not in ['jpg', 'jpeg', 'png', 'gif', 'webp']:
                messages.error(request, 'Недопустимый формат. Разрешены: JPG, PNG, GIF, WEBP')
                return redirect('profile')
...
def call_request(request):
    ...
    if form.is_valid():
        ...
        local_created_at = call_req.created_at + timedelta(hours=3)
        message = f"""
        Здравствуйте!
        ...
        │ Дата и время: {local_created_at.strftime('%d.%m.%Y %H:%M')}
        └─────────────────────────────────────┘
