...
class CustomUserCreationForm(UserCreationForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        today = date.today()
        max_date = today
        min_date = today - relativedelta(years=116)
    ...
    def clean_birth_date(self):
        ...
        if birth_date:
            ...
            allowed_min_date = eighteen_years_ago - timedelta(days=1)
            if birth_date > allowed_min_date:
                raise ValidationError('Регистрация доступна с 18 лет (на следующий день после дня рождения).')
        return birth_date
...
class ProfileUpdateForm(forms.ModelForm):
    ...
    def clean_telephone(self):
        ...
        if not re.match(r'^7\d{10}$', telephone_digits):
            raise ValidationError('Телефон должен быть в формате: +79991234567 или 89991234567')
        formatted = '+' + telephone_digits
    ...
    def clean_image(self):
        ...
        if image:
            ...
            try:
                ...
                img.verify()  # проверяет, что файл действительно изображение
            except Exception:
                raise ValidationError('Загрузите правильное изображение. Файл повреждён или не является изображением.')
        return image
class ReviewForm(forms.ModelForm):
    description = forms.CharField(
        ...
        widget=forms.Textarea(attrs={
            'class': 'form-control',
            'rows': 4,
            'placeholder': 'Напишите ваш отзыв (не более 255 символов)...'
        }),
        label='Отзыв'
    )
    class Meta:
